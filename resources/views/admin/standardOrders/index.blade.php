    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <div class="container">
            ...
            <table class="table table-bordered">
                ...
                @foreach ($standardOrders as $standardOrder )
                <tr>
                    @php
                        $mesures = explode(',', $standardOrder->mesure);
                    @endphp

                    <td> {{$standardOrder->categories->gender}} </td>
                    <td> {{$standardOrder->sub_categories->type}} </td>
                    <td>
                        @for ($i = 0; $i < count($mesures); $i++)
                            {{$mesures[$i]}},
                        @endfor
                     </td>
                    <td>  {{$standardOrder->doublure}} </td>
                    <td>  {{$standardOrder->description}} </td>
                    <td>  {{$standardOrder->bands->title}} </td>
                    <td>  {{$standardOrder->images}} </td>

                    <td>
                        <form action=" {{ route('destroyProduct', $standardOrder->id) }} " method="POST">
                                @csrf
                            <span class="edit">
